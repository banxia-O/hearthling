...
金币系统是 Hearthling 第一版更适合落地的内部经济层。

它不碰真实支付，也不需要银行、卡组织或支付 provider。

它的作用是让 agent 的日常行动产生资源感：

```text
做事 → 获得金币/物品 → 做选择 → 买东西 → 改变小院状态
```

金币不是为了做复杂经营游戏，而是为了让“生活”变得可循环。
...
```json
{
  "coins": 42,
  "shop": [
    {"id": "bait_basic", "name": "普通鱼饵", "price": 3, "effect": "fish_success + small"},
    {"id": "seed_pack", "name": "一包菜种", "price": 6, "effect": "plant_crop"},
    {"id": "tea_leaves", "name": "茶叶", "price": 8, "effect": "cook_tea"},
    {"id": "desk_repair", "name": "修书桌", "price": 30, "effect": "room_quality +1"}
  ]
}
```
...
- 金币只在 Hearthling 内部使用。
- 金币不能兑换真实钱。
- 金币消费不需要用户审批，但要写入 events.jsonl。
- 稀有/纪念性物品可以触发日记或主动消息。
- 金币系统服务生活感，不做抽卡、不做氪金、不做数值压迫。

和 Agent Wallet 的关系：

- Coins 是小院内部货币，适合 MVP。
- Agent Wallet 是现实世界预算接口，放在后期。
- 两者都要有交易记录，但 Coins 不涉及真实支付风险。
